...
class PyvaxCompileTool(Tool):
    ...
    async def run(self, ctx: "AgentContext", **kwargs: Any) -> ToolResult:
        ...
        # Use dual compiler: PyVax first → Solidity fallback
        from classified_agent.tools.dual_compiler import compile_flexible

        result = compile_flexible(source_file, workspace, optimizer_level=optimizer)

        if not result.success:
            return ToolResult(success=False, error=result.error or "Compilation failed.")
        ...
        # Include Solidity-specific extras when fallback was used
        if result.method == "solidity":
            output["solidity_source_saved"] = True
            if result.snowtrace_payload:
                output["snowtrace_payload"] = result.snowtrace_payload
